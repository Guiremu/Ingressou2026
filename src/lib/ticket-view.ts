...
export async function getTicketViewData(codigoQr: string) {
  ...
  const { data: ticket } = await admin
    .from("tickets")
    .select(
      "id, codigo_qr, assinatura_hmac, status, event_id, order_id, profile_id, titular_nome, intransferivel, ticket_types(nome), events(titulo, local, endereco, cidade, data_inicio, producers(nome_fantasia, razao_social)), orders(comprador_nome, profile_id)",
    )
  ...
  const orderInfo = ticket.orders as unknown as { comprador_nome: string; profile_id: string | null } | null;
