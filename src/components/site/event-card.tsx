  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <h3 className="line-clamp-2 font-semibold text-neutral-900">{event.titulo}</h3>
        <p className="text-sm text-neutral-500">{formatDate(event.data_inicio)}</p>
        <p className="text-sm text-neutral-500">{event.cidade}</p>
        <p className="mt-auto text-xs text-neutral-400">por {event.producerNome}</p>
      </div>
    </Link>
  );
}
